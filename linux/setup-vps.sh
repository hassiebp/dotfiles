...
TMUX_SERVER_PREFIX="${VPS_TMUX_SERVER_PREFIX-}"
...
Setup options:
  ...
  --tmux-prefix KEY               tmux prefix override for the server user; default keeps the shared prefix and uses F12 for nested tmux
...
phase_precheck() {
  ...
  if [[ -n "$TMUX_SERVER_PREFIX" ]]; then
    sc_info "  tmux_prefix_override=$TMUX_SERVER_PREFIX"
  else
    sc_info "  tmux_prefix_override=shared (use F12 nested toggle)"
  fi
...
phase_dotfiles() {
  ...
  if sc_run_as_user_cmd "$TARGET_USER" test -x "$install_dir/install.sh"; then
    ...
    local tmux_prefix_marker tmux_nested_marker
    ...
    if command -v tmux >/dev/null 2>&1; then
      ...
      tmux_prefix_marker="# Managed by setup-vps.sh (tmux-prefix)"
      tmux_nested_marker="# Managed by setup-vps.sh (tmux-nested)"
      manage_tmux_local=0
      if ! sc_run_as_user_cmd "$TARGET_USER" test -f "$tmux_local_conf"; then
        manage_tmux_local=1
      elif sc_run_as_user_cmd "$TARGET_USER" grep -Eq '^# Managed by setup-vps\.sh \(tmux-(prefix|nested)\)$' "$tmux_local_conf"; then
        manage_tmux_local=1
      else
      ...
      if ((manage_tmux_local)); then
        ...
        sc_register_tmp_path "$tmux_override_tmp"
        {
          if [[ -n "$TMUX_SERVER_PREFIX" ]]; then
            ...
            printf '%s\n' "$tmux_prefix_marker"
            printf "set -g @prefix-key '%s'\n" "$q_tmux_prefix"
            printf '%s\n' "unbind C-a"
            printf 'set -g prefix %s\n' "$q_tmux_prefix"
            printf 'bind %s send-prefix\n' "$q_tmux_prefix"
          else
            printf '%s\n' "$tmux_nested_marker"
            printf '%s\n' "# Keep the shared tmux prefix and use F12 to toggle outer tmux key handling."
          fi
        } > "$tmux_override_tmp"
        ...
        if [[ -n "$TMUX_SERVER_PREFIX" ]]; then
          ...
          sc_info "Configured $tmux_local_conf to keep the shared tmux prefix; use F12 for nested sessions"
        fi
      fi
